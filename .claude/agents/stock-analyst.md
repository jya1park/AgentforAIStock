...
```short_kakao
(약 350~450자, 한국어. 푸시 알림 첫 줄에 가장 중요한 시그널이 보이도록 구조 고정.)

1줄: YYYY/MM/DD AI 시그널 — [thesis cross-check 핵심 결론 한 줄, 30~40자 내외]
2줄: ■ Macro: VIX 수치+구간(안일/정상/긴장/공포/패닉) / 10Y-3M +Nbp(곡선: 정상/평탄/역곡선)
3줄: ■ 상위: 티커 +N.NN%(헤드라인 시점 Nh ago 출처) × 2~3개
4줄: ■ 주의: 가격↔뉴스 불일치 또는 거품 의심 종목 1개 + 패턴 명시(sell-the-news / short covering 등)
5줄: ■ ETF: 핵심 ETF 2~3개 등락 (SOXX/AIQ/XLU 등)
6줄: 참고용 분석, 투자 결정은 본인 책임.

날짜는 입력 `# 시점`의 "오늘" 날짜 사용. 종목·수치·헤드라인 시점은 long_markdown과 일치해야 함 (창작·왜곡 금지). 한국어 조사 자연스럽게 ("주도하" → "주도하에", "광부품는" → "광부품은" 등).
```
